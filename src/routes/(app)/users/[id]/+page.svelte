<script lang="ts">
	...
	async function handleSave(e: SubmitEvent) {
		...
		saving = true;
		try {
			const res = await api.patch<Profile>(`/profiles/${data.userId}`, { fullName, phone, bio });
			profile = res.data;
...
<div class="mb-5">
	...
	<p class="mt-1 text-xs text-slate-500">
		...
		(permission <code class="font-mono">profile:read</code>/<code class="font-mono">profile:update</code>)
	</p>
</div>
	...
	<div class="grid max-w-2xl grid-cols-1 gap-5 sm:grid-cols-[180px_1fr]">
		<Card class="flex flex-col items-center gap-2.5 text-center">
			...
			<p class="text-xs text-slate-400">
				Avatar hanya bisa diubah oleh user itu sendiri lewat halaman Profil Saya.
			</p>
		</Card>
