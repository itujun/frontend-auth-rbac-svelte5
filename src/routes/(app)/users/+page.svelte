...
<div class="mb-5">
	...
	<p class="mt-1 text-xs text-slate-500">
		<code class="font-mono">GET /users</code> (permission
		<code class="font-mono">user:read</code>) &middot; klik salah satu user
		untuk lihat/edit profile-nya (<code class="font-mono">profile:read</code
		>/<code class="font-mono">profile:update</code>)
	</p>
</div>
				...
				{#each rows as u (u.id)}
					<tr class="hover:bg-slate-50">
						...
						<td class="border-b border-slate-200 px-2.5 py-2.5 text-right">
							<RowAction
								href={resolve('/(app)/users/[id]', { id: u.id.toString() })}
								>Lihat profil</RowAction
							>
						</td>
					</tr>
				{/each}
