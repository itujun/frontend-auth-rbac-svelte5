<script lang="ts">
	...
	async function handleSaveRole(e: SubmitEvent) {
		...
		savingRole = true;
		try {
			const res = await api.patch<Role>(`/roles/${data.roleId}`, { name, description });
			role = res.data;
			pushToast('Role berhasil diperbarui', 'success');
		} catch (err) {
			pushToast(err instanceof ApiError ? err.message : 'Gagal menyimpan role', 'error');
		} finally {
			savingRole = false;
		}
	}
	...
	$effect(() => {
		...
		selectedPermissionIds = new Set(current.map((p) => p.id));
	});
	...
	async function handleSyncPermissions() {
		...
		try {
			...
			pushToast(err instanceof ApiError ? err.message : 'Gagal menyimpan permission', 'error');
		} finally {
	...
	// --- Assign/revoke user ---
	let roleUsers = $state<RoleUserSummary[]>([]);
	let newUserId = $state('');
	let assigning = $state(false);

	$effect(() => {
		roleUsers = data.users.value ?? [];
	});
	...
	async function handleAssignUser(e: SubmitEvent) {
		...
		try {
			...
			newUserId = '';
			const res = await api.get<RoleUserSummary[]>(`/roles/${data.roleId}/users`);
			roleUsers = res.data;
		} catch (err) {
			pushToast(err instanceof ApiError ? err.message : 'Gagal assign user', 'error');
		} finally {
			assigning = false;
		}
	}
	...
	async function handleRevokeUser(userId: number) {
		...
		try {
			...
			pushToast(err instanceof ApiError ? err.message : 'Gagal mencabut role', 'error');
		}
	}
</script>
...
{#if data.role.error}
	<div class="max-w-md rounded-md border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700">
		{data.role.error}
	</div>
{:else if role}
	<div class="flex max-w-3xl flex-col gap-5">
		<Card>
			...
			<form class="mt-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2" onsubmit={handleSaveRole}>
				<Input id="role-name" label="Nama" bind:value={name} required />
				<Input id="role-desc" label="Deskripsi" bind:value={description} />
				<div class="sm:col-span-2">
					<Button type="submit" loading={savingRole} disabled={savingRole}>Simpan</Button>
				</div>
			</form>
		</Card>
		...
		<Card>
			...
			<p class="mt-1 text-xs text-slate-500">
				...
				(permission <code class="font-mono">role:manage-permissions</code>) &mdash; centang lalu
				simpan untuk MENGGANTI seluruh daftar permission role ini.
			</p>
			...
			{:else}
				<div class="mt-3.5 grid grid-cols-1 gap-x-4 gap-y-1.5 sm:grid-cols-2 md:grid-cols-3">
		...
		<Card>
			...
			{:else}
				<form class="mt-3.5 flex items-end gap-2.5" onsubmit={handleAssignUser}>
					<div class="w-36">
						<Input id="new-user-id" label="User ID" bind:value={newUserId} placeholder="mis. 12" mono />
					</div>
					<Button type="submit" size="sm" loading={assigning} disabled={assigning}>Assign</Button>
				</form>

				{#if roleUsers.length === 0}
					<p class="mt-3.5 text-xs text-slate-400">Belum ada user dengan role ini.</p>
				{:else}
						...
						<tbody>
							{#each roleUsers as u (u.id)}
								<tr class="hover:bg-slate-50">
									<td class="border-b border-slate-200 px-2.5 py-2 font-mono">{u.id}</td>
									<td class="border-b border-slate-200 px-2.5 py-2">{u.email}</td>
